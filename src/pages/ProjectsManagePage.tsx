...
export function ProjectsManagePage() {
  ...
  const onAdd = async () => {
    ...
    if (!parsed.success) { notify.error(parsed.error.issues[0].message); return; }
    if (codeTaken(parsed.data.code)) { notify.error(`code "${parsed.data.code}" ซ้ำ`); return; }
    try {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <h2 className="font-display font-extrabold text-display">Manage Projects</h2>
      </div>
      ...
      <Card className="p-5 grid grid-cols-1 md:grid-cols-5 gap-3 items-end">
        ...
        <Button variant="primary" onClick={onAdd} disabled={create.isPending}>
          + Add project
        </Button>
      </Card>
...
function ProjectRow({ project: p, progress, isEditing, onToggleEdit, onSave, onDelete, onStatusChange }: ProjectRowProps) {
      ...
      <tr className="border-t border-cream-300">
        ...
        <td className="px-4 py-2 text-right">
          <div className="flex items-center justify-end gap-3">
            <button onClick={onToggleEdit} className="text-ink-700 hover:text-peri" title="Edit">
              <Pencil size={16} />
            </button>
            <button onClick={onDelete} className="text-ink-700 hover:text-rose" title="Delete">
              <Trash2 size={16} />
            </button>
          </div>
        </td>
  ...
  const parsed = ProjectDraftSchema.safeParse({
    ...
    dev_at:      form.dev_at || null,
    uat_at:      form.uat_at || null,
    golive_at:   form.golive_at || null,
  });
  ...
  const handleSave = () => {
    ...
    onSave({
      ...
      kickoff_at:  parsed.data.kickoff_at ?? null,
      dev_at:      parsed.data.dev_at ?? null,
      uat_at:      parsed.data.uat_at ?? null,
      golive_at:   parsed.data.golive_at ?? null,
    });
  };
    ...
    <tr className="border-t border-cream-300 bg-cream-50">
      <td colSpan={6} className="px-4 py-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <FieldEdit label="Name" error={fieldErr('name')}>
            <Input
              ...
              className={cn(fieldErr('name') && 'border-rose')}
            />
          </FieldEdit>
        ...
        <div className="mt-3 pt-3 border-t border-cream-300">
          <span className="font-display font-bold text-label text-ink-500 uppercase text-xs mb-2 block">
            Timeline Phases (optional)
          </span>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {(
              [
                { key: 'kickoff_at' as const, label: 'Kickoff' },
                { key: 'dev_at'     as const, label: 'Dev' },
                { key: 'uat_at'     as const, label: 'UAT' },
                { key: 'golive_at'  as const, label: 'Go-live' },
              ] as const
            ).map(({ key, label }) => (
              <FieldEdit key={key} label={label} error={fieldErr(key)}>
                <div className="flex items-center gap-1">
                  <DatePopover
                    value={form[key] || todayISO()}
                    onChange={(iso) => setForm({ ...form, [key]: iso })}
                  />
                  {form[key] && (
                    <button
                      type="button"
                      onClick={() => setForm({ ...form, [key]: '' })}
                      className="text-ink-500 hover:text-rose text-xs font-mono"
                    >
                      clear
                    </button>
                  )}
        ...
        <div className="flex gap-2 mt-4 justify-end">
          <Button variant="paper" size="sm" onClick={onToggleEdit}>
            <X size={14} /> Cancel
          </Button>
          <Button variant="primary" size="sm" onClick={handleSave} disabled={!parsed.success}>
            <Check size={14} /> Save
          </Button>
        </div>
      </td>
